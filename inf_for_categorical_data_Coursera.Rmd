...
- More than 50,000 men and women selected from 57 countries across the globe in five continents.
...
1.0 * nrow(us12[us12$response == 'atheist', ]) / nrow(us12)
```
...
- Independence: sampled observations must be independent from each other. 
    - The sampling is randomly conducted
    - If sampled without replacement, the sample size is less than 10% of the whole population
...
- ME is the distance away fromt the sampled proportion that the population porportion might fall in.
...
sqrt(( 1 - 0.0499) * 0.0499 / 1002) * 1.96
```
...
nrow(cn12)
# sample proportion
nrow(cn12[cn12$response == 'atheist', ]) / nrow(cn12)
# confidence interval
inference(y = response, data = cn12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
# margin of error
sqrt((1 - 0.47) * 0.47 / 500) * 1.96

# Germany
ds12 <- atheism %>%
  filter(nationality == "Germany" , atheism$year == "2012")
inference(y = response, data = ds12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
...
nrow(ds12)
# sample proportion
nrow(ds12[ds12$response == 'atheist', ]) / nrow(ds12)
# confidence interval
inference(y = response, data = ds12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
# margin of error
sqrt((1 - 0.15) * 0.15 / 502) * 1.96
```
...
table(sp05_12[sp05_12$year == "2005",]$response)
table(sp05_12[sp05_12$year == "2012",]$response)
...
inference(x = year, y = response, data = sp05_12, statistic = "proportion", type = "ht", null = 0, alternative = "twosided", method = "theoretical", success = "atheist")
```
...
us05_12 <- atheism %>%
  filter(nationality == "United States" , atheism$year >= 2005 & atheism$year <= 2012)

table(us05_12[us05_12$year == "2005",]$response)
table(us05_12[us05_12$year == "2012",]$response)
...
inference(x = year, y = response, data = us05_12, statistic = "proportion", type = "ht", null = 0, alternative = "twosided", method = "theoretical", success = "atheist")
```
