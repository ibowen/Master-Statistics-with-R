...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median = median(arr_delay), interquartile = IQR(arr_delay), n = n())
```
...
speed_distance <- nycflights %>% 
    mutate(avg_speed = distance / air_time) %>%
    select(avg_speed, distance)
ggplot(data = speed_distance, aes(x = distance, y = avg_speed)) +
    geom_point()

ggplot(data = speed_distance, aes(x = distance)) +
    geom_histogram()
```
...
# type your code for Question 9 here, and Knit
nycflights %>%
    mutate(arr_type = ifelse(arr_delay <= 0, "on time", "delayed"), dep_type = ifelse(dep_delay < 5, "on time", "delayed")) %>%
    group_by(dep_type) %>%
    summarize(on_time_rate = sum(arr_type == "on time") / n())
```