...
ames %>%
    sample_n(size = 100) %>%
    summarise(price_bar = mean(price))
```
...
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_means50 <- data.frame()
for (i in 1:5000) {
    mean <- ames %>%
    sample_n(size = 50) %>%
    summarise(price_bar = mean(price))
    sample_means50[i, 1] <- mean
}

ggplot(data = sample_means50, aes(x = price_bar)) +
...
# type your code for this Exercise here, and Run Document
sample_means150 <- data.frame()
for (i in 1:5000) {
    mean <- ames %>%
    sample_n(size = 150) %>%
    summarise(price_bar = mean(price))
    sample_means150[i, 1] <- mean
}

ggplot(data = sample_means150, aes(x = price_bar)) +
    geom_histogram(binwidth = 500)
```
...
# type your code for this Exercise here, and Run Document
sample_means15 <- data.frame()
for (i in 1:2000) {
    mean <- ames %>%
    sample_n(size = 15) %>%
    summarise(price_bar = mean(price))
    sample_means15[i, 1] <- mean
}

ggplot(data = sample_means15, aes(x = price_bar)) +
    geom_histogram(binwidth = 1000)

sample_means15 %>% summarise(population_mean = mean(price_bar))
```
...
sample_means150 %>% summarise(population_mean = mean(price_bar))
```
...
sample_means15 %>% summarize(mu = mean(price_bar), sigma = sd(price_bar))
sample_means150 %>% summarize(mu = mean(price_bar), sigma = sd(price_bar))
```
