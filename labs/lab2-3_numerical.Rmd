...
```{r str}
str(nc)
ggplot(data = nc, aes(x = 1, y = weight)) +
    geom_boxplot()
```
...
Reject Null hypothesis since the p-value is lower than the significant level
</div>
...
# not reject null hypothesis, since the p-value is bigger than 0.05
```
...
inference(y = mage, x = mature, data = nc, statistic = "mean", type = "ci", method = "theoretical")
```
...
```{r pick-your-own}
# type your code for the Exercise here, and Knit
# find out if the birth weights are related to premature.
# H0: mu_full term =  mu_premie
# HA: mu_full term != mu_premie
# P-value is extremly lower than the siginificant level of 5%, then we reject the H0 and accept HA, that is:
# The average birth weights are different for premature and non-premature babies.
# condition check: 
# 1.independent within groups, and sample size < 10% of the population
# 2.indepentdent between groups
